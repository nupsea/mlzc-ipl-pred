...
def prep_data():
    ...
    print(
        f" Data set size: Second innings:{len(second_innings)}, Matches: {len(matches_df)}"
    )

    gdf = second_innings.sort_values(["match_id", "over", "ball"]).reset_index(
        drop=True
    )

    # Rolling aggregate of last 2 overs
    gdf["runs_last_12_balls"] = gdf.groupby("match_id", group_keys=False)[
        "total_runs"
    ].apply(lambda x: x.shift(1).rolling(window=12, min_periods=1).sum())

    gdf["wickets_last_12_balls"] = gdf.groupby("match_id", group_keys=False)[
        "is_wicket"
    ].apply(lambda x: x.shift(1).rolling(window=12, min_periods=1).sum())

    print("Merged data of matches and deliveries..")
    merged_df = gdf.merge(matches_df, how="left", left_on="match_id", right_on="id")
    ...
    merged_df["balls_remaining"] = (merged_df.target_overs * 6) - (
        merged_df.over * 6 + merged_df.ball
    )
...
if __name__ == "__main__":
    ...
    cols_of_interest = (
        low_cardinal_features + high_cardinal_features + numeric_features + ["chase"]
    )
    ...
    df_full_train, df_test = train_test_split(
        main_df, test_size=0.2, shuffle=False, random_state=1
    )
    ...
    model = LogisticRegression(
        C=0.01, max_iter=1000, random_state=21, class_weight="balanced"
    )
    ...
    print(
        f"Trained Accuracy={np.mean(fold_accs):.4f}, ROC AUC={np.mean(fold_aucs):.4f}"
    )
